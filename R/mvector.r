#' @rdname marray
#' @export
mvector <- function(datatype = c("double", "float", "integer"), length, filename, readonly) {
  ...
  new("mvector", ptr = ptr, file = filename, length = as.integer(length), datatype = datatype, readonly = readonly)
}
